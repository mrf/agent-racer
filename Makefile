.PHONY: dev run build test test-race test-frontend test-e2e lint ci deps clean embed build-frontend validate-embed dist tui tui-build tui-deps tui-test tui-lint coverage coverage-race coverage-frontend tui-coverage
