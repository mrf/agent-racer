...
func (m *Monitor) poll() {
	...
	for _, src := range m.sources {
		srcUpdates, srcActiveKeys := m.pollSource(src, now)
...
func (m *Monitor) pollSource(src Source, now time.Time) (updates []*session.SessionState, activeKeys map[string]bool) {
